...
from app.utils.fixes import apply_clahe, reduce_highlights, unsharp_mask, upscale_bicubic_to_min, denoise_fastnlmeans
from app.utils.detect import detect_single_face
from app.utils.constants import MIN_BORDERLINE_RES, MIN_RESOLUTION, MAX_ALLOWED_RES, ALLOWED_FORMATS
from PIL import Image
from io import BytesIO
import cv2

def validate_and_fix_image(image_bytes: bytes) -> bytes:
    """
    Main pipeline that:
    1. Reads image
    2. Validates format
    ...
    7. Returns improved image bytes for the model
    """
    ...
    if fmt not in ALLOWED_FORMATS:
        raise HTTPException(status_code=400, detail=f"Unsupported format: {pil.format}. Allowed: JPG/JPEG, PNG")
    ...
    if not face_found:
        ...
        if min_dim < MIN_BORDERLINE_RES:
            raise HTTPException(status_code=400, detail=f"No face detected and image resolution too low ({w}x{h})")
        # Face missing but resolution borderline or fine → still reject
        raise HTTPException(status_code=400, detail="No face detected. Please upload single, clear face.")
    
    if len(faces)>1:
        raise HTTPException(status_code=400, detail="Multiple faces detected. Please upload a single face.")
    ...
    severe = [
        ...
        and v.get("status").startswith("severe")
    ]
    ...
    if min(w2,h2) < MIN_BORDERLINE_RES:
        raise HTTPException(status_code=400, detail=f"Image too small after fixes ({w2}x{h2}).")

    # Return bytes as JPEG to be passed to model
    return pil_bytes_from_cv2(img_fixed, fmt="JPEG", quality=92)
