...
def pil_bytes_from_cv2(img_cv: np.ndarray, fmt="JPEG", quality=90) -> bytes:
    ...
    pil = Image.fromarray(img_rgb)                          # Convert numpy array to PIL image
    buf = BytesIO()                                         # Create BytesIO buffer
    pil.save(buf, format=fmt, quality=quality)              # Save PIL image to bytes to buffer
